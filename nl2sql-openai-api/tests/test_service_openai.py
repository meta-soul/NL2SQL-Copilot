...
import toml
from nl2sql.service.openai import OpenaiService
...
for name, service_args in conf['openai']['service'].items():
    if service_args.get('type') != "openai":
        continue

    service = OpenaiService(name, service_args['conf'])
    #service = create_service('openai', service_args['conf'])
