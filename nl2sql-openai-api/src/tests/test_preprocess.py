...
from nl2sql.schema.base import database_stringify, table_stringify, column_stringify
from nl2sql.service.pre_processor import PreProcessorChatPayloadParser, PreProcessorPromptParser, PreProcessorPromptGenerator

class DummyService:

    def __init__(self):
        self.database_stringify = database_stringify
        self.database_stringify_kwargs = {}
        self.table_stringify = table_stringify
        self.table_stringify_kwargs = {}
        self.column_stringify = column_stringify
        self.column_stringify_kwargs = {}
        self.make_prompt = lambda schema, question: str(schema) + "\n" + str(question)
        self.make_prompt_kwargs = {}
...
context = {}
context['service'] = DummyService()

database_stringify, table_stringify, column_stringify
payload = {'model': 'gpt-3.5-turbo', 'messages': [{'role': 'user', 'content': content}]}

pre1 = PreProcessorChatPayloadParser()
res = pre1(payload, context=context)
context.update(res)
print(res)

pre2 = PreProcessorPromptParser(prompt_version="v3")
res = pre2(payload, context=context)
context.update(res)
print(res)

pre3 = PreProcessorPromptGenerator()
res = pre3(payload, context=context)
context.update(res)
print(res)
