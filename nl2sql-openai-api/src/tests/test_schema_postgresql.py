...
from nl2sql.schema.postgresql import TableSchema, DatabaseSchema

s = r"""CREATE TABLE alignment (
...
COMMENT ON COLUMN gender.gender IS '列名：性别种类；注释：超级英雄的性别，性别种类有：男、女、无'
 ...
 CONSTRAINT hero_attribute_hero_id_fkey FOREIGN KEY(hero_id) REFERENCES superhero(id),
 CONSTRAINT hero_attribute_attribute_id_fkey FOREIGN KEY(attribute_id) REFERENCES attribute(id)
 );
...
COMMENT ON COLUMN hero_attribute.attribute_value IS '列名：属性值；注释：属性值\n常识：\n如果超级英雄在某一特定属性上具有较高的属性值，则意味着与其他超级英雄相比，他们在该领域更加熟练或强大。 例如，如果超级英雄具有较高的力量属性值，他们可能能够举起更重的物体或比其他超级英雄打出力量更大的拳头。'
 ...
 CONSTRAINT hero_power_hero_id_fkey FOREIGN KEY(hero_id) REFERENCES superhero(id),
 CONSTRAINT hero_power_power_id_fkey FOREIGN KEY(power_id) REFERENCES superpower(id)
 );
