...
from transformers import AutoTokenizer,AutoConfig,XLMRobertaModel,RobertaModel
...
class MyClassifier(nn.Module):
    def __init__(
        ...
        model_class = XLMRobertaModel if "xlm" == model_type else RobertaModel
...
def init_schema_linking_model(model_path: str, device: str, num_gpus: int, **kwargs):
    set_seed(42)
    if device == "cpu":
        kwargs = {}
    elif device == "gpu":
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path, 
        add_prefix_space=True, cache_dir=kwargs.get("cache_dir"))
    ...
    state = torch.load(
        model_path + "/dense_classifier.pt",
